...
-- The following columns have null values: "Country", "worlwide_gross_income", "languages" and "production_company"
...
-- The total number of movies released each month:
...
SELECT Count(DISTINCT id) AS movies, year
FROM movie
...
SELECT Count(*) AS movies_belong_to_one_genre
FROM movies_belong_to_one_genre; 
...
SELECT production_company, sum(r.total_votes) AS vote_count, DENSE_RANK() OVER(ORDER BY sum(r.total_votes) DESC) AS prod_comp_rank
FROM movie m
INNER JOIN ratings r 
ON m.id= r.movie_id
GROUP BY production_company
...
+---------------+-------------------+---------------------+----------------------+-----------------+*/
-- Type your code below:
WITH top_indian_actor
     AS (SELECT nm.NAME AS actor_name, Sum(r.total_votes) AS total_votes, Count(DISTINCT rm.movie_id) AS movie_count,
...
(
       SELECT *, Datediff(next_date_published, date_published) AS date_diff
